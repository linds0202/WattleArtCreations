        ...
        <div className={scroll ? "relative p-4 bg-[#262626] rounded-3xl" : "relative p-4"}>
          <div className={scroll ? "overflow-x-auto flex-nowrap pb-4 flex justify-between horz-scroll-con relative rounded-2xl" : "p-4 flex justify-around relative"}>
            {info.map(el =>
                <div 
                  key={el.style} 
                  className={scroll ? 'w-[30%] h-[500px] mx-4 flex flex-col items-center p-4 hover:cursor-pointer' : 'w-3/12 flex flex-col items-center drip-card hover:cursor-pointer'} 
                  style={scroll ? {flex: '0 0 auto'} : {}} onClick={() => router.push(`/portraits/?styleOne=${el.abrv}`)}
                >
